...
from transformers.pytorch_utils import find_pruneable_heads_and_indices, is_torch_greater_or_equal_than_1_10, prune_linear_layer
...
class ViltEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if image_embeds is None:
            ...
            for i in range(4):
                image_embed, image_mask, patch_index = self.visual_embed(
                    pixel_values[:, i], pixel_mask, max_image_length=self.config.max_image_length
                )
                image_embeds.append(image_embed)
                image_masks.append(image_mask)
            image_embeds = torch.cat(image_embeds, dim=1)
            image_masks = torch.cat(image_masks, dim=1)'''
        else:
...
class TextEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        #print('input_shape:',input_shape[0],   input_shape[1])
        ...
        if self.position_embedding_type == "absolute":
            ...
            #embeddings[:, :position_embeddings.shape[1]] = embeddings[:, :position_embeddings.shape[1]].clone() + position_embeddings.clone()
...
class ViltSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        #change
        #print("self attention output shape",outputs)
...
#CHANGE self attention to afno

class ViltAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = ViltSelfAttention(config)#change self-attention
        # self.attention = AFNO1D(hidden_size=config.hidden_size, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1)
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        # print("attention mask",attention_mask,"head mask",head_mask,"output attentions",output_attentions)#change print
        #ipdb.set_trace()
        ...
        #selfoutputs=1
        #ipdb.set_trace()
        #print("attention output shape",outputs)
        return outputs
...
class ViltLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        #change
        #self.attention = AFNO2D(hidden_size=config.hidden_size, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1)
        self.attention = AFNO1D(hidden_size=config.hidden_size, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1)
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        self_attention_outputs = self.attention(
            ...
            #attention_mask,
            #head_mask,
            #output_attentions=output_attentions,#change
        )
        #change 替换后加ipdb看下输出是否只需要self attention outputs即可
        ...
        #outputs = (layer_output,) + outputs#change
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
               ...
               #layer_outputs = layer_module(hidden_states, attention_mask, layer_head_mask, output_attentions)change
                #print(i, hidden_states.shape)
                layer_outputs = layer_module(hidden_states, attention_mask, layer_head_mask)
            hidden_states=layer_outputs
            #hidden_states = layer_outputs[0] #change
...
class ViltForMaskedLM(ViltPreTrainedModel):
    ...
    def forward(
        ...
        #print("batch",input_ids.shape)#change
