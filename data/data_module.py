...
from .processor import get_dataset, KGProcessor
...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        #print(features[0].keys(),'key',features[0])
        ...
        full_img, pixel_images=[], []
        #for entity in tqdm(entities):
        for entity in entities:#change
            ...
            f=open(self.entity_img_path, 'rb')
            info=pickle.load(f)
            if en_file in info:
                en_imgs = info[en_file]
                full_img = Image.open(en_imgs).convert('RGB').resize((384, 384))
                full_img=vilt_processor.feature_extractor(full_img, return_tensors='pt',pad_and_return_pixel_mask=False).pixel_values.squeeze()
                pixel_images.append(full_img)
            else:
                pixel_images.append(torch.zeros((3, 384, 384)))
           # print(entity)
        features['pixel_values']=torch.stack(pixel_images)
        # print(features['pixel_values'])
        features.update(features_keys)
...
class KGC(BaseDataModule):
    def __init__(self, args, model) -> None:
        ...
        #self.tokenizer.save("dataset/"+self.args.task_name+"/tokenizer.json")
        #path="dataset/"+self.args.task_name
        #self.tokenizer.from_pretrained(path, use_fast = True)

        print(f"add_special_tokens succ ")
        entity_img_path = {'wn18': 'dataset/wn18_best_img.pickle', 'fb15k-237': 'dataset/fb15k_best_img.pickle'}[self.args.task_name]
        #entity_img_files = listdir(entity_img_path)
        print(entity_img_path)
...
#process dataset
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument("--model_name_or_path", type=str, default="bert-base-uncased", help="the name or the path to the pretrained model")
        parser.add_argument("--data_dir", type=str, default="dataset/FB15k-237", help="the name or the path to the pretrained model")
    ...
    def train_dataloader(self):
        return DataLoader(self.data_train, num_workers=self.num_workers, pin_memory=False, collate_fn=self.sampler, batch_size=self.args.batch_size, drop_last=True,shuffle=self.args.pretrain)
